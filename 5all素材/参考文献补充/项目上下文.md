...
| 数据集 | 说明 | 当前状态 |
|--------|------|----------|
| AnyVisLoc | 18,000 张无人机图像，7 种 DJI 机型，15 城市 25 区域，30m–300m 高度，20°–90° 俯仰角 | 待下载 |
| 参考图（航空） | 124 张，分辨率 0.02–0.35m，SfM 构建 | 待下载 |
| 参考图（卫星） | 72 张，分辨率 0.13–0.55m，Google Earth + ALOS 30m DSM | 待下载 |
| University-1652 | 检索模型预训练权重 | 待下载 |
...
| 70°-90°（正下视） | 103 | 89.3% | 95.1% | 96.1% | 11.7m | 2.5m |

结论：正下视精度最高（A@5m=92.9%），倾斜视角精度下降（A@5m=79.4%），与论文 Figure 7 结论一致。
...
| 场景 | 图像数 | 平均误差 | A@5m | A@10m | A@20m |
|------|--------|---------|------|-------|-------|
| QZ_SongCity | 286 | 84.52m | 18.9% | 49.0% | 73.8% |
| Qingzhou_3_2 | 59 | 33.25m | 6.8% | 37.3% | 62.7% |
| QingZhou_2024 | 142 | 158.90m | 5.6% | 21.1% | 43.7% |
| **总计** | **487** | **100.00m** | **13.6%** | **39.4%** | **63.7%** |

注意：卫星图精度远低于航空图（A@5m 13.6% vs 82.3%），主要因 DSM 分辨率低（30m vs 0.02m）。
...
| 第五章 | 实验验证与结果分析 | 我们的实验结果 | ✅ 齐全 |
| 第六章 | 总结与展望 | 实验结论 | ✅ 齐全 |

### 参考素材

1. **论文笔记**: paper/notes.md（359 行，详细技术分析）
2. **微信公众号文章**: CVPR 2026 AnyVisLoc 详细介绍（行业背景、数据集、算法对比）
3. **Gemini 3.1 Pro 参考文本**: 前四章完整内容（含双光融合数学模型、EKF 滤波方程）
4. **实验结果**: 航空图/卫星图/策略对比/俯仰角分析/匹配方法对比
